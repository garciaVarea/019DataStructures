...
public class HelloController {
    ...
    protected void onHelloButtonClick() {
        ...
        for (Document doc : collection.find()) {
            ObjectId movieIdObj = doc.getObjectId("movie_id");
            String movieId = movieIdObj != null ? movieIdObj.toString() : null;
            datosList.add(new Datos(doc.getObjectId("_id").toString(), doc.getString("name"), doc.getString("email"), movieId, doc.getString("text"), doc.getDate("date")));
        }
